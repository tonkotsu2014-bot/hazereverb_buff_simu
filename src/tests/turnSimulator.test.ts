...
import { ParsedCharacterData } from '../logic/wikiParser';
...
describe('Turn Simulator Logic', () => {
    ...
    it('should generate correct actions for a 3-person party over 2 rounds', () => {
        ...
        // Round 1
        expect(actions[0]).toEqual({ round: 1, globalTurn: 1, actorIndex: 0, actorName: 'Alice' });
        // Alice is Attacker (non-supporter), so Boss acts immediately after
        expect(actions[1]).toEqual({ round: 1, globalTurn: 2, actorIndex: -1, actorName: 'Boss' });
        expect(actions[2]).toEqual({ round: 1, globalTurn: 3, actorIndex: 1, actorName: 'Bob' });
        expect(actions[3]).toEqual({ round: 1, globalTurn: 4, actorIndex: 2, actorName: 'Charlie' });

        // Round 2
        expect(actions[4]).toEqual({ round: 2, globalTurn: 5, actorIndex: 0, actorName: 'Alice' });
        expect(actions[5]).toEqual({ round: 2, globalTurn: 6, actorIndex: -1, actorName: 'Boss' });
        expect(actions[6]).toEqual({ round: 2, globalTurn: 7, actorIndex: 1, actorName: 'Bob' });
        expect(actions[7]).toEqual({ round: 2, globalTurn: 8, actorIndex: 2, actorName: 'Charlie' });
    });
    ...
    it('should handle a full party of 9', () => {
        ...
        expect(actions[4]).toEqual({ round: 1, globalTurn: 5, actorIndex: 4, actorName: 'Char5' }); // Attacker
        expect(actions[5]).toEqual({ round: 1, globalTurn: 6, actorIndex: -1, actorName: 'Boss' }); // Boss acts after Char5
        expect(actions[9]).toEqual({ round: 1, globalTurn: 10, actorIndex: 8, actorName: 'Char9' }); // Last Supporter
    });
